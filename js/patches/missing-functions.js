// ══════════════════════════════════════════════════════════════════════════
        ...
        async function doFullBackup() {
            try {
                ...
                history.unshift({
                    date: new Date().toISOString(), type: 'manual', filename,
        ...
        async function loadReportStudents() {
            ...
            if (sel) {
                sel.innerHTML = '<option value="">— Select Student —</option>' +
                    students.map(s => `<option value="${s.id}">${esc(s.first_name + ' ' + s.last_name)}</option>`).join('');
            }
        }
        ...
        async function generateReportCard() {
            ...
            const type = document.getElementById('report-type')?.value || 'endterm'; // default to endterm

            if (!classId || !studentId) {
                showToast('Select class and student', 'warning');
                return;
            }
            ...
            const subjects = (state.subjects || [])
                .filter(s => (s.level || '').toLowerCase() === (cls?.level || '').toLowerCase() && s.is_active !== false)
                .sort((a, b) => (a.sort_order || 99) - (b.sort_order || 99));

            // ── Build a map of marks by assessment_id for O(1) lookups ──
            const marksMap = new Map();
            (state.marks || [])
                .filter(m => assessments.some(a => a.id === m.assessment_id))
            ...
            const subjectRows = subjects.map(sub => {
                ...
                const mgAssessments = assessments.filter(a => a.subject_id === sub.id && !['Exam', 'Final Exam'].includes(a.assessment_type));
                const exAssessments = assessments.filter(a => a.subject_id === sub.id && ['Exam', 'Final Exam'].includes(a.assessment_type));

                // ── Calculate MG ──
                let mg = null;
                if (mgAssessments.length > 0) {
                    let totalRaw = 0, totalMaxRaw = 0, count = 0;
                    for (const a of mgAssessments) {
                        const mark = marksMap.get(a.id + '-' + student.id);
                        if (mark !== undefined) {
                            totalRaw += mark.score;
                            totalMaxRaw += a.max_marks;
                            count++;
                        }
                    }
                    if (count > 0) {
                        const avgRaw = totalRaw / count;
                        const avgMax = totalMaxRaw / count;
                        mg = avgMax > 0 ? (avgRaw / avgMax) * mgMax : 0;
                    }
                }

                // ── Calculate EX ──
                let ex = null;
                if (exAssessments.length > 0) {
                    let totalRaw = 0, totalMaxRaw = 0, count = 0;
                    for (const a of exAssessments) {
                        const mark = marksMap.get(a.id + '-' + student.id);
                        if (mark !== undefined) {
                            totalRaw += mark.score;
                            totalMaxRaw += a.max_marks;
                    ...
                    if (count > 0) {
                        const avgRaw = totalRaw / count;
                        const avgMax = totalMaxRaw / count;
                        ex = avgMax > 0 ? (avgRaw / avgMax) * exMax : 0;
                    }
                }
                ...
                const total = (mg !== null || ex !== null) ? (mg || 0) + (ex || 0) : null;
                if (total !== null) {
                    totalScore += total;
                    totalMax += mgMax + exMax;
                }

                return { sub, mg, ex, total, mgMax, exMax };
            });

            const overallPct = totalMax > 0 ? (totalScore / totalMax) * 100 : null;
            const grade = overallPct !== null ? getGrade(overallPct) : '—';
            const rank = await calculateStudentRank(student.id, parseInt(classId));

            const school = state.schoolSettings || {};

            // ── Build report data for QR ──
            const reportData = {
                className: cls?.name || '—',
                termName: term?.name || '—',
                ...
                subjects: subjectRows.map(r => ({
                    ...
                    ex: r.ex,
                    total: r.total,
                    max: (r.mgMax || 0) + (r.exMax || 0),
                    pct: r.total !== null ? (r.total / ((r.mgMax || 0) + (r.exMax || 0)) * 100) : null,
                    grade: r.total !== null ? getGrade((r.total / ((r.mgMax || 0) + (r.exMax || 0))) * 100) : '—'
                }))
            };
            ...
            if (content) {
                ...
                content.innerHTML = buildReportCardHTML(student, cls, term, subjectRows, overallPct, grade, rank, school, isNursery, type);

                // ── Inject QR code ──
                const reportEl = content.querySelector('.report-card');
                if (reportEl) {
                    addQRCodeToReport(reportEl, student, reportData, type);
                }
            }
        }

        function buildReportCardHTML(student, cls, term, subjectRows, pct, grade, rank, school, isNursery, type) {
            ...
            if (pct !== null) {
                if (isAnnual) {
                    const promotionMark = parseFloat(school.promotion_mark || school.pass_mark || 50);
                    const passed = pct >= promotionMark;
                    decisionHtml = passed
                    ...
                    const passMark = parseFloat(school.pass_mark || 50);
                    const passed = pct >= passMark;
                    decisionHtml = passed
                        ? `<div style="margin:12px 0;padding:10px 16px;background:#d1fae5;border-radius:8px;text-align:center;font-weight:700;color:#065f46;font-size:.95rem">
                    ✅ ${isNursery ? 'PROGRÈS SATISFAISANT' : 'SATISFACTORY PROGRESS'}
                   </div>`
                        : `<div style="margin:12px 0;padding:10px 16px;background:#fef3c7;border-radius:8px;text-align:center;font-weight:700;color:#92400e;font-size:.95rem">
                    ⚠️ ${isNursery ? 'PROGRÈS À AMÉLIORER' : 'NEEDS IMPROVEMENT'}
                   </div>`;
                }
            }

            // ── Build subject rows ──
            const subjectRowsHtml = subjectRows.map(r => {
                const totalMax = (r.mgMax || 0) + (r.exMax || 0);
                const pct = r.total !== null ? (r.total / totalMax) * 100 : null;
                const gradeBadge = r.total !== null
                    ? `<span class="badge ${getGradeClass(pct)}">${getGrade(pct)}</span>`
            ...
            <td style="text-align:center;font-weight:700">${r.total !== null ? r.total.toFixed(1) : '—'}</td>
        ...
        <div class="report-header">
            ...
            <div class="report-header-text">
                ...
                <h3>${isAnnual ? 'ANNUAL REPORT' : (type === 'midterm' ? 'MID-TERM REPORT' : 'END OF TERM REPORT')} — ${esc(term?.name || '')}</h3>
            </div>
        </div>
        ...
        <table class="report-subjects">
            <thead>
                <tr>
                    ...
                    <th style="text-align:center">MG<br><small>/${subjectRows[0]?.mgMax || 50}</small></th>
                    <th style="text-align:center">EX<br><small>/${subjectRows[0]?.exMax || 50}</small></th>
                    <th style="text-align:center">TOT<br><small>/${(subjectRows[0]?.mgMax || 50) + (subjectRows[0]?.exMax || 50)}</small></th>
        ...
        <div class="report-summary">
            <div>
                <div class="summary-label">${isNursery ? 'TOTAL GÉNÉRAL' : 'GRAND TOTAL'}</div>
                <div class="summary-value">${pct !== null ? (pct.toFixed(1) + '%') : '—'}</div>
            </div>
            <div>
                <div class="summary-label">${isNursery ? 'COTE' : 'GRADE'}</div>
                <div class="summary-value">${grade}</div>
            </div>
            <div>
                <div class="summary-label">${isNursery ? 'RANG' : 'RANK'}</div>
        ...
        <div class="report-footer">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;text-align:center;margin-top:12px">
                <div>
                    <strong>${isNursery ? "Signature de l'enseignant(e)" : 'Teacher Signature'}</strong><br>
                    <div style="margin-top:24px;border-top:1px solid #ccc;padding-top:4px">${esc(school.school_name || '')}</div>
                </div>
                <div>
                    <strong>${isNursery ? "Cachet de l'école" : 'School Stamp'}</strong><br>
                    <div style="margin-top:40px"></div>
                </div>
                <div>
                    <strong>${isNursery ? "Signature du/de la directeur(trice)" : 'Head Teacher Signature'}</strong><br>
                    <div style="margin-top:24px;border-top:1px solid #ccc;padding-top:4px">${esc(school.head_teacher || school.report_footer_line2 || 'UWAYO GANZA Eugene')}</div>
                </div>
            </div>
        ...
        /**
 * Generate ALL report cards for a class in parallel for speed
 * Shows progress modal and auto-downloads combined PDF
 */
        async function generateAllReports() {
            ...
            showProgressModal('Generating Reports', `Generating ${total} report cards...`, 0);
            ...
            const results = await Promise.all(students.map(async (student, index) => {
                try {
                    const data = await generateSingleReport(student.id, classId, reportType, termId);
                    updateProgressModal(index + 1, total, `Generating: ${student.first_name} ${student.last_name}`);
                    return { student, data, success: true, index: index + 1 };
                } catch (error) {
                    console.error(`Failed for ${student.first_name} ${student.last_name}:`, error);
                    return { student, success: false, error: error.message, index: index + 1 };
                }
            }));
            ...
            showToast(`✅ Generated ${successResults.length} report${successResults.length !== 1 ? 's' : ''}${failedResults.length ? ` (${failedResults.length} failed)` : ''}`,
                failedResults.length > 0 ? 'warning' : 'success');

            // ── Auto-download combined PDF ──
            const cls = getClassById(classId);
            const term = getTermById(termId);
            await downloadAsCombinedPDF(successResults, cls, term);

            // ── Show batch summary ──
            if (failedResults.length > 0) {
                console.warn('Failed students:', failedResults.map(r => `${r.student.first_name} ${r.student.last_name}: ${r.error}`));
            }
        }

        /**
         * Show a progress modal for long-running operations
         * @param {string} title - Modal title
         * @param {string} message - Status message
         * @param {number} progress - Current progress (0-100)
         */
        function showProgressModal(title, message, progress = 0) {
            ...
            modal.style.cssText = 'z-index:9999;display:flex;align-items:center;justify-content:center';
            modal.innerHTML = `
        <div class="modal" style="max-width:400px">
            ...
            <div class="modal-body">
                <p id="progress-message" style="margin-bottom:12px">${message}</p>
                <div style="background:var(--border-light);border-radius:99px;height:8px;overflow:hidden">
                    <div id="progress-bar" style="width:${progress}%;height:100%;background:var(--role-primary);transition:width .3s ease"></div>
                </div>
                <p id="progress-text" style="font-size:12px;color:var(--text-muted);margin-top:8px;text-align:center">${progress}%</p>
            </div>
        </div>
    `;
        ...
        async function executeCarryForward() {
            ...
            for (const p of preview) {
                await insert('student_fees', {
                    student_id: p.student?.id, fee_category_id: p.fee.fee_category_id,
        ...
        function exportStatisticsData() {
            ...
            const data = classes.map(cls => {
                ...
                return {
                    'Class': cls.name, 'Students': students.length, 'Assessments': assessments.length,
        ...
        async function loadAttendanceSummary() {
            ...
            div.innerHTML = `
                ...
                <div class="table-wrapper"><table class="data-table">
                    ...
                    <tbody>${rows.map(r => `<tr>
                        <td><strong>${esc(r.s.first_name + ' ' + r.s.last_name)}</strong></td>
        ...
        async function loadAttendanceAnalytics() {
            ...
            if (!attendanceData.length) {
                (state.marks || []).forEach(m => {
                    ...
                    if (a) attendanceData.push({
                        student_id: m.student_id, class_id: a.class_id,
                ...
                if (classCtx && classStats.length) {
                    ...
                    window._attClassChart = new Chart(classCtx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: 'Attendance %', data: classStats.map(c => c.rate),
                ...
                if (trendCtx && trendMonths.length) {
                    ...
                    window._attTrendChart = new Chart(trendCtx, {
                        ...
                        data: {
                            labels: trendMonths,
                            datasets: [{
                                label: 'Monthly Rate %', data: trendRates,
        ...
        function exportTeacherPerformance() {
            const data = (state.teachers || []).map(t => {
                ...
                return {
                    'Teacher': t.name, 'Role': t.role, 'Status': t.is_active === false ? 'Inactive' : 'Active',
        ...
        async function sendAnnouncementFromModal(editId) {
            ...
            if (editId) {
                ...
                await insert('announcements', {
                    title, message, type, recipients, is_read: false,
