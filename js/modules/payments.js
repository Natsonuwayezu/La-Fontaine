// ══════════════════════════════════════════════════════════════════════════
        ...
        async function renderRecordPayment(el) {
            ...
            /**
 * Process a payment and automatically generate/download receipt PDF
 * @param {boolean} printAfter - If true, also open print dialog
 */
            async function processPayment(printAfter = false) {
                ...
                const schoolSettings = state.schoolSettings || {};
                ...
                const recordedBy = state.currentUser ? { role: state.currentUser.role, name: state.currentUser.name } : null;
                ...
                const receiptData = {
                    ...
                    fees: feeDetails,
                    schoolName: schoolSettings.school_name || 'ECOLE LA FONTAINE',
                    schoolAddress: schoolSettings.school_address || 'Rubavu, Rwanda',
                    logo: schoolSettings.school_logo || '🏫'
                };

                // ── Download PDF automatically ──
                try {
                    await downloadReceiptPDF(receiptData);
                    showToast(`✅ Payment of ${fmtCurrency(amount)} recorded (${receiptNum}) — Receipt downloaded`, 'success');
                } catch (pdfErr) {
                    console.warn('[Receipt PDF] Failed:', pdfErr);
                    showToast(`✅ Payment recorded (${receiptNum}) — PDF download failed, but payment is saved`, 'warning');
                }

                // ── If print requested (for physical copy) ──
                if (printAfter) {
                    setTimeout(() => {
                    ...
                    .total{font-size:20px;font-weight:bold;text-align:center;margin:15px 0;padding:10px;background:#d1fae5;border-radius:8px}
                    @media print{body{padding:0}}</style></head>
                    <body><div class="receipt"><h2 style="text-align:center">🏫 ECOLE LA FONTAINE</h2>
                    <p style="text-align:center">OFFICIAL PAYMENT RECEIPT</p><hr>
                    <div class="row"><span>Receipt #:</span><strong>${receiptNum}</strong></div>
                    <div class="row"><span>Date:</span><strong>${new Date(date).toLocaleDateString()}</strong></div>
