...
// Source lines: 26285–26587 of original monolith
// ============================================================
        ...
        function renderFeeStatusRows(rows) {
            return rows.map((r,i) => {
                ...
                return `<tr data-student-id="${r.s.id}" data-name="${esc(r.s.last_name+' '+r.s.first_name)}" data-class="${esc(cls?.name||'')}" data-status="${r.status}">
                    ...
                    <td>
                        <button class="btn-link" onclick="navigateToWithData('student-details',{studentId:${r.s.id}})" style="background:none;border:none;color:var(--role-primary);cursor:pointer;font-weight:600;text-align:left">
                            ${esc(r.s.last_name+' '+r.s.first_name)}
                        </button><br>
