// js/patches/missing-functions-98.js
// Source lines: 32950–36582 of original monolith
// ============================================================
        ...
        async function saveMarks() {
            ...
            showToast('⏳ Saving marks…', 'info', 2000);
            for (const input of inputs) {
                ...
                if (r.success) { saved++; input.style.background = 'var(--success-bg)'; }
                else { errors++; input.style.background = 'var(--danger-bg)'; }
            }
            await refreshTable('marks');
            await notifyAction('marks_import', { message: saved + ' marks saved by ' + (state.currentUser?.name || 'teacher') }, ['admin', 'teacher']);
            showToast('✅ ' + saved + ' marks saved' + (errors ? ' (' + errors + ' errors)' : ''), errors ? 'warning' : 'success');
        }
        ...
        async function loadMEStudentsTable() {
            ...
            tbody.innerHTML = students.map(student => {
                ...
                return '<tr><td>' + esc(student.first_name) + ' ' + esc(student.last_name) + '</td><td>' + esc(student.student_code || '—') + '</td><td style="text-align:center"><input type="number" class="mark-input form-control" style="width:80px;text-align:center" data-student-id="' + student.id + '" data-assessment-id="' + (assessmentId || '') + '" data-max="' + (assessment?.max_marks || 100) + '" value="' + (mark?.score ?? '') + '" min="0" max="' + (assessment?.max_marks || 100) + '" step="0.5"></td><td style="text-align:center">' + (assessment?.max_marks || '—') + '</td><td style="text-align:center">' + (mark ? '<span class="badge badge-success">Entered</span>' : '<span class="badge badge-neutral">Pending</span>') + '</td></tr>';
        ...
        async function loadMarksDatabase() {
            ...
            const data = state.marks.map(m => { const a = state.assessments.find(x => x.id === m.assessment_id); const s = state.students.find(x => x.id === m.student_id); const sub = state.subjects.find(x => x.id === a?.subject_id); const cls = state.classes.find(x => x.id === a?.class_id); return { id: m.id, student: s ? s.first_name + ' ' + s.last_name : '—', class: cls?.name || '—', subject: sub?.name || '—', assessment: a?.title || '—', score: m.score, max: a?.max_marks || '—', pct: a?.max_marks ? ((m.score / a.max_marks) * 100).toFixed(1) + '%' : '—', date: fmtDate(m.created_at) }; });
            container.innerHTML = '<div class="table-wrapper"><table class="data-table"><thead><tr><th>Student</th><th>Class</th><th>Subject</th><th>Assessment</th><th>Score</th><th>Max</th><th>%</th><th>Date</th></tr></thead><tbody>' + data.map(d => '<tr><td>' + esc(d.student) + '</td><td>' + esc(d.class) + '</td><td>' + esc(d.subject) + '</td><td>' + esc(d.assessment) + '</td><td style="text-align:right">' + d.score + '</td><td style="text-align:right">' + d.max + '</td><td style="text-align:center">' + d.pct + '</td><td>' + d.date + '</td></tr>').join('') + '</tbody></table></div>';
        }
            ...
            for (const a of assessments) {
                ...
                for (const s of students) {
                    ...
                    rows.push({
                        ...
                        'Date': fmtDate(a.date), 'Student': (s.last_name + ' ' + s.first_name).trim(),
                        'Score': mark?.score ?? '—',
        ...
        async function exportClassTimetable() {
            ...
            const rows = slots.map(s => {
                ...
                return { 'Day': s.day_of_week, 'Time': s.time_slot, 'Subject': subj?.name || '—', 'Code': subj?.code || '—', 'Teacher': teacher ? (teacher.last_name + ' ' + teacher.first_name).trim() : '—' };
            });
            ...
            showModal(`<div class="modal-overlay" id="add-slot-modal"><div class="modal modal-sm">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group"><label>Teacher</label>
                        ...
                        ${(state.teachers || []).map(t => `<option value="${t.id}">${esc(t.last_name + ' ' + t.first_name)}</option>`).join('')}</select></div>
                </div>
                <div class="modal-footer">
            ...
            showModal(`<div class="modal-overlay" id="edit-slot-modal"><div class="modal modal-sm">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group"><label>Teacher</label>
                        ...
                        ${(state.teachers || []).map(t => `<option value="${t.id}"${t.id === slot.teacher_id ? ' selected' : ''}>${esc(t.last_name + ' ' + t.first_name)}</option>`).join('')}</select></div>
                </div>
                <div class="modal-footer">
        ...
        async function detectAllConflicts() {
            ...
            const rows = conflicts.map((c, i) => {
                ...
                return `<tr><td>${esc(t ? t.last_name + ' ' + t.first_name : '—')}</td>
        ...
        async function exportConflictReport() {
            ...
            for (const slot of allSlots) {
                ...
                if (seen[key]) {
                    ...
                    rows.push({ 'Teacher': t ? t.last_name + ' ' + t.first_name : '—', 'Day': slot.day_of_week, 'Time': slot.time_slot, 'Subject 1': s1?.name || '—', 'Subject 2': s2?.name || '—' });
                } else seen[key] = slot;
            }
        ...
        async function loadAttendanceStudents() {
            ...
            const rows = students.map(s => {
                ...
                return `<tr>
                    <td>${esc(s.last_name + ' ' + s.first_name)}</td>
        ...
        async function saveAttendance() {
            ...
            if (abDiv && abBody) { abDiv.style.display = ''; abBody.innerHTML = absentList.length ? absentList.map(s => `<div style="padding:6px 0;border-bottom:1px solid var(--border-light)">${esc(s.last_name + ' ' + s.first_name)}</div>`).join('') : '<p style="color:var(--text-muted)">🎉 No absent students today!</p>'; }
        }
        ...
        async function exportAttendanceDay() {
            ...
            const cls = state.classes.find(c => String(c.id) === String(classId));
            const students = (state.students || []).filter(s => s.class_id === parseInt(classId) && s.status === 'Active');
            const rows = students.map(s => ({ 'Date': date, 'Class': cls?.name || '—', 'Student': s.last_name + ' ' + s.first_name, 'Status': document.getElementById('att-status-' + s.id)?.value || '—', 'Notes': document.getElementById('att-note-' + s.id)?.value || '' }));
        ...
        async function generateAttReport() {
            ...
            const rows = Object.entries(byStudent).map(([sid, counts]) => {
                ...
                return `<tr><td>${esc(student ? student.last_name + ' ' + student.first_name : '—')}</td><td>${esc(cls?.name || '—')}</td>
        ...
        async function exportAttReport() {
            ...
            const rows = records.map(rec => {
                ...
                return { 'Date': rec.date, 'Class': cls?.name || '—', 'Student': student ? student.last_name + ' ' + student.first_name : '—', 'Status': rec.status, 'Notes': rec.notes || '' };
            });
        ...
        function renderStudentTable(students) {
            ...
            tbody.innerHTML = students.map(s => {
                ...
                return `<tr>
                    ...
                    <td><a href="#" onclick="navigateToWithData('student-details',{student_id:${s.id}});return false" style="font-weight:600;color:var(--primary)">${esc((s.last_name + ' ' + s.first_name).trim())}</a></td>
        ...
        function openEditStudentModal(studentId) {
            ...
            showModal(`<div class="modal-overlay" id="edit-student-modal"><div class="modal">
                <div class="modal-header"><h3>✏️ Edit Student — ${esc(s.last_name + ' ' + s.first_name)}</h3>
        ...
        async function deleteStudentPrompt(studentId) {
            ...
            if (!await confirmDialog(`Archive student ${s.last_name} ${s.first_name}? They can be restored later.`)) return;
            ...
            if (r.success) {
                ...
                await logActivity(state.currentUser?.id, state.currentUser?.role, 'Archived student: ' + s.last_name + ' ' + s.first_name, 'students', studentId);
        ...
        async function executeBulkExport() {
            ...
            if (type === 'students') {
                ...
                rows = marks.map(m => { const a = assessments.find(x => x.id === m.assessment_id); const s = state.students.find(x => x.id === m.student_id); const subj = state.subjects.find(x => x.id === a?.subject_id); return { 'Student': s ? s.last_name + ' ' + s.first_name : '—', 'Assessment': a?.title || '—', 'Subject': subj?.name || '—', 'Score': m.score, 'Max': a?.max_marks, 'Absent': m.is_absent ? 'Yes' : 'No' }; });
                filename = 'Marks_Export';
            } else if (type === 'payments') {
                let payments = state.payments || []; if (termId) payments = payments.filter(p => String(p.term_id) === termId);
                rows = payments.map(p => { const s = state.students.find(x => x.id === p.student_id); return { 'Receipt': p.receipt_number, 'Date': fmtDate(p.payment_date), 'Student': s ? s.last_name + ' ' + s.first_name : '—', 'Amount': p.amount, 'Method': p.payment_method, 'Notes': p.notes || '' }; });
                ...
                rows = records.map(rec => { const s = state.students.find(x => x.id === rec.student_id); return { 'Date': rec.date, 'Student': s ? s.last_name + ' ' + s.first_name : '—', 'Class': (state.classes.find(c => c.id === rec.class_id))?.name || '—', 'Status': rec.status, 'Notes': rec.notes || '' }; });
        ...
        async function renderStudentFeesTable() {
            ...
            const rows = students.map(s => {
                ...
                return `<tr><td><a href="#" onclick="navigateToWithData('student-fees',{fee_student_id:${s.id}});return false" style="font-weight:600">${esc((s.last_name + ' ' + s.first_name).trim())}</a></td>
        ...
        async function exportStudentFeeBalances() {
            ...
            const rows = students.map(s => {
                ...
                return { 'Student': s.last_name + ' ' + s.first_name, 'Class': cls?.name || '', 'Total Fees': bal.total || 0, 'Paid (RWF)': bal.paid || 0, 'Balance (RWF)': bal.balance || 0, 'Status': bal.balance <= 0 ? 'Paid' : bal.paid > 0 ? 'Partial' : 'Unpaid' };
            });
        ...
        function filterPaymentHistoryTable() {
            ...
            if (search) payments = payments.filter(p => { const student = state.students.find(s => s.id === p.student_id); return (p.receipt_number || '').toLowerCase().includes(search) || (student ? (student.last_name + ' ' + student.first_name).toLowerCase().includes(search) : false); });
            ...
            tbody.innerHTML = payments.map(p => {
                ...
                return `<tr><td><code>${esc(p.receipt_number || '—')}</code></td><td>${fmtDate(p.payment_date)}</td><td>${esc(student ? student.last_name + ' ' + student.first_name : '—')}</td><td>${esc(cls?.name || '—')}</td><td style="text-align:right;font-weight:600">${fmtCurrency(p.amount)}</td><td><span class="badge badge-info">${esc(p.payment_method || '—')}</span></td><td><button class="btn btn-sm btn-outline" onclick="window.printReceiptById&&window.printReceiptById(${p.id})">🧾</button></td></tr>`;
            }).join('');
        }
        ...
        async function exportFullPaymentHistory() {
            ...
            const rows = (state.payments || []).map(p => {
                ...
                return { 'Receipt': p.receipt_number, 'Date': fmtDate(p.payment_date), 'Student': student ? student.last_name + ' ' + student.first_name : '—', 'Class': cls?.name || '—', 'Term': term?.name || '—', 'Amount (RWF)': p.amount, 'Method': p.payment_method, 'Recorded By': p.recorded_by || '—', 'Notes': p.notes || '' };
            });
        ...
        function filterReceipts() {
            ...
            if (dateTo) payments = payments.filter(p => (p.payment_date || '') <= dateTo);
            if (method) payments = payments.filter(p => p.payment_method === method);
            if (search) payments = payments.filter(p => { const s = state.students.find(x => x.id === p.student_id); return (p.receipt_number || '').toLowerCase().includes(search) || (s ? (s.last_name + ' ' + s.first_name).toLowerCase().includes(search) : false); });
        ...
        function renderFullReceiptsList(payments) {
            ...
            container.innerHTML = `<div class="table-wrapper"><table class="data-table"><thead><tr><th>Receipt #</th><th>Date</th><th>Student</th><th>Class</th><th>Amount (RWF)</th><th>Method</th><th>Actions</th></tr></thead>
                ...
                const student = state.students.find(s => s.id === p.student_id); const cls = state.classes.find(c => c.id === student?.class_id);
                return `<tr><td><code>${esc(p.receipt_number || '—')}</code></td><td>${fmtDate(p.payment_date)}</td><td>${esc(student ? student.last_name + ' ' + student.first_name : '—')}</td><td>${esc(cls?.name || '—')}</td><td style="text-align:right;font-weight:600">${fmtCurrency(p.amount)}</td><td><span class="badge badge-info">${esc(p.payment_method || '—')}</span></td><td><button class="btn btn-sm btn-outline" onclick="window.printReceiptById&&window.printReceiptById(${p.id})">🖨️ Print</button></td></tr>`;
            }).join('')}
                </tbody></table></div>`;
        }
        ...
        function exportReceiptsList() {
            const rows = (state.payments || []).map(p => {
                const student = state.students.find(s => s.id === p.student_id); const cls = state.classes.find(c => c.id === student?.class_id);
                return { 'Receipt #': p.receipt_number, 'Date': fmtDate(p.payment_date), 'Student': student ? student.last_name + ' ' + student.first_name : '—', 'Class': cls?.name || '—', 'Amount': p.amount, 'Method': p.payment_method, 'Notes': p.notes || '' };
            });
        ...
        async function printReceipt(paymentId) {
            ...
            <div class="row"><span>Student:</span><span>${esc(student ? student.last_name + ' ' + student.first_name : '—')}</span></div>
        ...
        async function filterReversalPayments() {
            ...
            if (search) reversals = reversals.filter(r => { const s = state.students.find(x => x.id === r.student_id); return (r.original_receipt || '').toLowerCase().includes(search) || (s ? (s.last_name + ' ' + s.first_name).toLowerCase().includes(search) : false); });
            ...
            container.innerHTML = reversals.map(rev => {
                ...
                return `<tr><td>${fmtDate(rev.created_at)}</td><td><code>${esc(rev.original_receipt || '—')}</code></td><td>${esc(student ? student.last_name + ' ' + student.first_name : '—')}</td><td style="text-align:right;color:var(--danger)">${fmtCurrency(rev.amount || 0)}</td><td>${esc(rev.reason || '—')}</td><td>${esc(rev.reversed_by || '—')}</td></tr>`;
            }).join('');
        }
        ...
        async function exportReversalHistory() {
            ...
            const rows = reversals.map(rev => {
                const student = state.students.find(s => s.id === rev.student_id);
                return { 'Date': fmtDate(rev.created_at), 'Receipt': rev.original_receipt || '—', 'Student': student ? student.last_name + ' ' + student.first_name : '—', 'Amount': rev.amount || 0, 'Reason': rev.reason || '', 'By': rev.reversed_by || '' };
            });
        ...
        async function viewReversalDetails(reversalId) {
            ...
            showModal(`<div class="modal-overlay" id="rev-detail-modal"><div class="modal modal-sm"><div class="modal-header"><h3>↩️ Reversal Details</h3><button class="modal-close" onclick="closeModal('rev-detail-modal')">✕</button></div>
                <div class="modal-body"><table class="data-table"><tbody>
                    ...
                    <tr><td><strong>Student</strong></td><td>${esc(student ? student.last_name + ' ' + student.first_name : '—')}</td></tr>
        ...
        function exportBulkPaymentTemplate() {
            ...
            const rows = studentIds.map(sid => {
                ...
                return { student_code: s?.student_code || '', student_name: s ? s.last_name + ' ' + s.first_name : '—', amount: bal.balance || 0, payment_method: 'Cash', payment_date: today, notes: '' };
            });
        ...
        async function renderFeeAssignmentsTable() {
            ...
            container.innerHTML = `<div class="table-wrapper"><table class="data-table"><thead><tr><th>Student</th><th>Class</th><th>Category</th><th>Amount</th><th>Paid</th><th>Status</th><th>Actions</th></tr></thead>
                ...
                return `<tr><td>${esc(s ? s.last_name + ' ' + s.first_name : '—')}</td><td>${esc(cls?.name || '—')}</td><td>${esc(cat?.name || '—')}</td><td>${fmtCurrency(f.amount || 0)}</td><td>${fmtCurrency(f.paid_amount || 0)}</td>
        ...
        async function exportFeeAssignments() {
            ...
            const rows = (state.studentFees || []).map(f => {
                const s = state.students.find(x => x.id === f.student_id); const cls = state.classes.find(c => c.id === s?.class_id); const cat = state.feeCategories.find(x => x.id === f.fee_category_id);
                return { 'Student': s ? s.last_name + ' ' + s.first_name : '—', 'Class': cls?.name || '—', 'Category': cat?.name || '—', 'Amount': f.amount || 0, 'Paid': f.paid_amount || 0, 'Balance': (f.amount || 0) - (f.paid_amount || 0), 'Status': f.is_paid ? 'Paid' : f.is_waived ? 'Waived' : 'Pending', 'Due Date': fmtDate(f.due_date) };
            });
                        ...
                        <div class="form-grid">
                            <div class="form-group full">
                                ...
                                <select id="sw-student" class="form-control" onchange="window._loadWaiverFees()">
                                    ...
                                    ${students.map(s=>`<option value="${s.id}" ${prefillStudentId==s.id?'selected':''}>${esc(s.last_name+' '+s.first_name)} (${esc(getClassById(s.class_id)?.name||'—')})</option>`).join('')}
                                </select>
                            </div>
                            <div class="form-group full">
        ...
        function openFullWaiverModal() {
            ...
            showModal(`<div class="modal-overlay" id="full-waiver-modal"><div class="modal modal-sm"><div class="modal-header"><h3>🎁 Full Fee Waiver</h3><button class="modal-close" onclick="closeModal('full-waiver-modal')">✕</button></div>
                <div class="modal-body"><div class="alert alert-warning">⚠️ Waives ALL outstanding fees for this student.</div>
                    <div class="form-group"><label>Student *</label><select id="fwv-student" class="form-control"><option value="">— Select Student —</option>${students.map(s => `<option value="${s.id}">${esc(s.last_name + ' ' + s.first_name)}</option>`).join('')}</select></div>
        ...
        async function renderFeeTermTable() {
            ...
            const rows = students.map(s => {
                ...
                return `<tr><td>${esc(s.last_name + ' ' + s.first_name)}</td><td>${esc(cls?.name || '—')}</td><td style="text-align:right">${fmtCurrency(totalOwed)}</td><td style="text-align:right">${fmtCurrency(totalPaid)}</td><td style="text-align:right;color:${balance > 0 ? 'var(--danger)' : 'var(--success)'};font-weight:600">${fmtCurrency(balance)}</td><td><span class="badge ${balance <= 0 ? 'badge-success' : totalPaid > 0 ? 'badge-warning' : 'badge-danger'}">${balance <= 0 ? '✅ Clear' : totalPaid > 0 ? '⚡ Partial' : '⏳ Unpaid'}</span></td></tr>`;
            }).join('');
        ...
        async function exportFeeTermStatus() {
            ...
            const rows = (state.students || []).filter(s => s.status === 'Active').map(s => {
                ...
                return { 'Student': s.last_name + ' ' + s.first_name, 'Class': cls?.name || '', 'Owed': owed, 'Paid': paid, 'Balance': balance, 'Status': balance <= 0 ? 'Clear' : paid > 0 ? 'Partial' : 'Unpaid' };
            });
        ...
        async function exportBalancesToExcel() {
            ...
            const rows = (state.students || []).filter(s => s.status === 'Active').map(s => {
                const cls = state.classes.find(c => c.id === s.class_id); const bal = typeof getFullStudentBalance === 'function' ? getFullStudentBalance(s.id) : { total: 0, paid: 0, balance: 0 };
                return { 'Student': s.last_name + ' ' + s.first_name, 'Class': cls?.name || '', 'Total (RWF)': bal.total || 0, 'Paid (RWF)': bal.paid || 0, 'Balance (RWF)': bal.balance || 0, 'Status': (bal.balance || 0) <= 0 ? 'Clear' : (bal.paid || 0) > 0 ? 'Partial' : 'Unpaid' };
            });
        ...
        function editTeacher(teacherId) {
            ...
            showModal(`<div class="modal-overlay" id="edit-teacher-modal"><div class="modal modal-sm"><div class="modal-header"><h3>✏️ Edit — ${esc(t.last_name + ' ' + t.first_name)}</h3><button class="modal-close" onclick="closeModal('edit-teacher-modal')">✕</button></div>
                <div class="modal-body"><div class="form-grid">
        ...
        async function deleteTeacher(teacherId) {
            ...
            if (!await confirmDialog(`Delete ${t.last_name} ${t.first_name}? Cannot be undone.`)) return;
        ...
        function resetTeacherPassword(teacherId) {
            const t = (state.teachers || []).find(x => x.id === teacherId); if (!t) return;
            showModal(`<div class="modal-overlay" id="reset-pw-modal"><div class="modal modal-sm"><div class="modal-header"><h3>🔑 Reset Password — ${esc(t.last_name + ' ' + t.first_name)}</h3><button class="modal-close" onclick="closeModal('reset-pw-modal')">✕</button></div>
                <div class="modal-body">
        ...
        function markStudentAbsent(studentId) {
            const s = state.students.find(x => x.id === studentId); if (!s) return;
            const today = new Date().toISOString().split('T')[0];
            showModal(`<div class="modal-overlay" id="mark-absent-modal"><div class="modal modal-sm"><div class="modal-header"><h3>❌ Mark Absent — ${esc(s.last_name + ' ' + s.first_name)}</h3><button class="modal-close" onclick="closeModal('mark-absent-modal')">✕</button></div>
                <div class="modal-body">
